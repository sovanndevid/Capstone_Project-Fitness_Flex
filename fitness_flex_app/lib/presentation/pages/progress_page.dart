...
class _ProgressPageState extends State<ProgressPage> {
        ...
        child: ListView(
          ...
          children: [
            ...
            FutureBuilder<List<dynamic>>(
              future: Future.wait([_todayWorkoutMinsHourlyF, _todayCaloriesHourlyF]),
              builder: (context, snap) {
                final loading = snap.connectionState != ConnectionState.done;
                final workoutHourly = !loading && snap.hasData ? (snap.data![0] as List<double>) : List<double>.filled(24, 0);
                final calHourly = !loading && snap.hasData ? (snap.data![1] as List<Map<String, double>>) : List.generate(24, (_) => {'in': 0.0, 'out': 0.0});

                final minsToday = workoutHourly.fold<double>(0, (a, b) => a + b);
                final inToday = calHourly.fold<double>(0, (a, m) => a + (m['in'] ?? 0));
                final outToday = calHourly.fold<double>(0, (a, m) => a + (m['out'] ?? 0));
                ...
                return _heroHeader(
                  ...
                  loading: loading,
                  kpis: [
                    _KpiData(label: "Calories In", value: "${inToday.round()}", color: Colors.orange, icon: Icons.restaurant),
                    _KpiData(label: "Calories Out", value: "${outToday.round()}", color: Colors.green, icon: Icons.local_fire_department),
                    _KpiData(label: "Workout Min", value: "${minsToday.round()}", color: AppTheme.primaryColor, icon: Icons.fitness_center),
                  ],
                  badge: balance == 0 ? "On Track" : (balance > 0 ? "+$balance kcal" : "$balance kcal"),
                );
              },
            ),
            ...
            FutureBuilder<List<dynamic>>(
              future: Future.wait([_todaySummaryF, _goalF, _weeklyWorkoutMinsF]),
              builder: (context, snap) {
                final loading = snap.connectionState != ConnectionState.done;
                if (loading) return _loadingBox(height: 96);
                final Map<String, double> summary = snap.data![0] as Map<String, double>;
                final NutritionGoal? goal = snap.data![1] as NutritionGoal?;
                final List<int> weeklyMins = snap.data![2] as List<int>;
                final msgs = _buildMotivationMessages(summary, goal, weeklyMins);
            ...
            AnimatedSwitcher(
              ...
              child: _rangeIndex == 0
                  ? Column(
                      ...
                      children: [
                        FutureBuilder<List<double>>(
                          ...
                          builder: (context, snap) {
                            final loading = snap.connectionState != ConnectionState.done;
                            final data = !loading && snap.hasData ? snap.data! : List<double>.filled(24, 0.0);
                            return _sectionCard(
                              title: 'Workout Minutes (today)',
                              icon: Icons.fitness_center,
                              child: loading ? _loadingBox() : _buildWorkoutLineChartHourly(data),
                            );
                          },
                        ),
                        const SizedBox(height: 12),
                        FutureBuilder<List<Map<String, double>>>(
                          ...
                          builder: (context, snap) {
                            final loading = snap.connectionState != ConnectionState.done;
                            final data = !loading && snap.hasData ? snap.data! : List.generate(24, (_) => {'in': 0.0, 'out': 0.0});
                            return _sectionCard(
                              title: 'Calories In vs Out (today)',
                              icon: Icons.local_fire_department,
                              child: loading ? _loadingBox() : _buildCaloriesBarChartHourlyBuckets(data),
                            );
                          },
                        ),
                      ],
                  ...
                  : Column(
                      ...
                      children: [
                        FutureBuilder<List<dynamic>>(
                          future: Future.wait([_weeklyWorkoutMinsF, _weeklyCaloriesInOutF]),
                          builder: (context, snap) {
                            final loading = snap.connectionState != ConnectionState.done;
                            final mins = !loading && snap.hasData ? (snap.data![0] as List<int>) : const <int>[];
                            final cal = !loading && snap.hasData ? (snap.data![1] as List<Map<String, double>>) : const <Map<String, double>>[];
                            return Column(
                              children: [
                                _sectionCard(
                                  title: 'Workout Minutes (7 days)',
                                  icon: Icons.timeline,
                                  child: loading ? _loadingBox() : _buildWorkoutLineChart(mins),
                                ),
                                const SizedBox(height: 12),
                                _sectionCard(
                                  title: 'Calories In vs Out (7 days)',
                                  icon: Icons.stacked_bar_chart,
                                  child: loading ? _loadingBox() : _buildCaloriesBarChart(cal),
                                ),
                              ],
                            );
                          },
            ...
            FutureBuilder<List<dynamic>>(
              future: Future.wait([_todaySummaryF, _goalF, _weeklyWorkoutMinsF, _todayActivityProgressF]),
              builder: (context, snapshot) {
                final loading = snapshot.connectionState != ConnectionState.done;
                final Map<String, double> summary = !loading && snapshot.hasData ? snapshot.data![0] as Map<String, double> : {};
                final NutritionGoal? goal = !loading && snapshot.hasData ? snapshot.data![1] as NutritionGoal : null;
                final List<int> weeklyMins = !loading && snapshot.hasData ? snapshot.data![2] as List<int> : const <int>[];
                final Map<String, double> act = !loading && snapshot.hasData ? snapshot.data![3] as Map<String, double> : const {'steps': 0.0, 'hydration': 0.0};
                ...
                return Column(
                  children: [
                    _sectionCard(
                      ...
                      child: loading ? _loadingBox(height: 140) : _buildMacroPieChart(summary),
                    ),
                    const SizedBox(height: 12),
                    _sectionCard(
                      ...
                      child: loading
                          ...
                          : _buildProgressRings(
                              workouts: weeklyMins.where((m) => m > 0).length / 5.0,
  ...
  Future<Map<String, double>> _fetchTodayActivityProgress() async {
    ...
    final stepTarget = (user.data()?['dailyStepsTarget'] as num?)?.toDouble() ?? 10000;
    final waterTarget = (user.data()?['dailyWaterTargetMl'] as num?)?.toDouble() ?? 2000;
    ...
    final steps = sSnap.docs.fold<num>(0, (a, d) => a + ((d.data()['steps'] as num?) ?? 0)).toDouble();
    final water = hSnap.docs.fold<num>(0, (a, d) => a + ((d.data()['ml'] as num?) ?? 0)).toDouble();

    return {
      'steps': stepTarget > 0 ? (steps / stepTarget).clamp(0, 1).toDouble() : 0,
      'hydration': waterTarget > 0 ? (water / waterTarget).clamp(0, 1).toDouble() : 0,
    };
  }
  ...
  Future<List<double>> _fetchTodayWorkoutMinsHourly() async {
    ...
    final snap = await _db
        .collection('users').doc(_uid).collection(_colWorkouts)
        .where('completedAt', isGreaterThanOrEqualTo: Timestamp.fromDate(start))
        .where('completedAt', isLessThan: Timestamp.fromDate(end))
        .get();
  ...
  Future<List<Map<String, double>>> _fetchTodayCaloriesInOutHourly() async {
    ...
    final mealsSnap = await _db
        .collection('users').doc(_uid).collection(_colNutrition)
        .where('timestamp', isGreaterThanOrEqualTo: Timestamp.fromDate(start))
        .where('timestamp', isLessThan: Timestamp.fromDate(end))
        .get();
  ...
  // Line: weekly workout minutes
  Widget _buildWorkoutLineChart(List<int> weeklyMins) {
    final spots = List.generate(7, (i) => FlSpot(i.toDouble(), (i < weeklyMins.length ? weeklyMins[i] : 0).toDouble()));
    return SizedBox(
      height: 240,
      child: LineChart(
        LineChartData(
          ...
          titlesData: FlTitlesData(
            bottomTitles: AxisTitles(sideTitles: SideTitles(showTitles: true, getTitlesWidget: (v, _) => _dayLabel(v.toInt()), interval: 1)),
  ...
  // Bar: calories in vs out
  Widget _buildCaloriesBarChart(List<Map<String, double>> weekly) {
    final groups = List.generate(7, (i) {
      final double inCal = i < weekly.length ? (weekly[i]['in'] ?? 0.0) : 0.0;
      final double outCal = i < weekly.length ? (weekly[i]['out'] ?? 0.0) : 0.0;
      return BarChartGroupData(
        x: i,
        barsSpace: 4,
        barRods: [
          BarChartRodData(toY: inCal.toDouble(), color: Colors.orange, width: 8, borderRadius: BorderRadius.circular(3)),
          BarChartRodData(toY: outCal.toDouble(), color: Colors.green, width: 8, borderRadius: BorderRadius.circular(3)),
        ],
      );
    });
    ...
    return SizedBox(
      ...
      child: PieChart(
        PieChartData(
          ...
          sections: [
            PieChartSectionData(value: p, color: Colors.red, title: 'P ${(p / total * 100).toStringAsFixed(0)}%', radius: 60, titleStyle: const TextStyle(fontSize: 12, color: Colors.white)),
            PieChartSectionData(value: c, color: Colors.green, title: 'C ${(c / total * 100).toStringAsFixed(0)}%', radius: 60, titleStyle: const TextStyle(fontSize: 12, color: Colors.white)),
            PieChartSectionData(value: f, color: Colors.blue, title: 'F ${(f / total * 100).toStringAsFixed(0)}%', radius: 60, titleStyle: const TextStyle(fontSize: 12, color: Colors.white)),
          ],
        ),
      ),
    );
  ...
  Widget _buildStatsAndStreaks(List<int> weeklyMins) {
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Text('Stats & Summaries', style: TextStyle(fontWeight: FontWeight.w600)),
        const SizedBox(height: 8),
        GridView.count(
          crossAxisCount: 2,
          shrinkWrap: true,
          ...
          children: [
            statCard(Icons.event_available, 'Total Sessions', '$totalSessions', AppTheme.primaryColor),
            statCard(Icons.schedule, 'Total Minutes', '$totalMinutes min', Colors.indigo),
            statCard(Icons.fitness_center, 'Strongest Lift', strongestLift, Colors.red),
            statCard(Icons.directions_run, 'Longest Run', longestRun, Colors.green),
          ],
        ),
      ],
    );
  ...
  Widget _buildMilestones() {
    final items = [
      {'icon': Icons.emoji_events, 'title': '10 Workouts', 'color': Colors.amber},
      {'icon': Icons.calendar_month, 'title': '1 Month Streak', 'color': Colors.purple},
      {'icon': Icons.directions_run, 'title': '100 km Run', 'color': Colors.green},
    ];

    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Text('Milestones & Achievements', style: TextStyle(fontWeight: FontWeight.w600)),
        const SizedBox(height: 8),
        SizedBox(
          height: 100,
            ...
            itemBuilder: (context, i) {
              ...
              return Container(
                ...
                decoration: BoxDecoration(
                  ...
                  border: Border.all(color: (it['color'] as Color).withOpacity(0.25)),
                ),
                padding: const EdgeInsets.all(12),
                child: Row(
                  children: [
                    ...
                    const SizedBox(width: 12),
                    Expanded(child: Text(it['title'] as String, style: const TextStyle(fontWeight: FontWeight.w600))),
                  ],
                ),
              );
            },
  ...
  // Motivation & guidance
  Widget _buildMotivationBlock(Map<String, double> summary, NutritionGoal? goal, List<int> weeklyMins) {
    ...
    final msg = <String>[
      if (goal != null) "You’re ${(calories / (goal.dailyCalories == 0 ? 1 : goal.dailyCalories) * 100).clamp(0, 100).toStringAsFixed(0)}% toward today’s calories goal — keep going!",
      "Just ${weeklyMins.where((m) => m > 0).length >= 3 ? 2 : 1} workouts away from your next badge.",
      if (needProtein) "Protein looks low today — consider a lean protein meal."
    ];
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const Text('Motivation & Tips', style: TextStyle(fontWeight: FontWeight.w600)),
            const SizedBox(height: 8),
            ...msg.map((m) => Padding(
  ...
  Widget _dayLabel(int i) {
    ...
    return Padding(padding: const EdgeInsets.only(top: 4), child: Text(days[i], style: const TextStyle(fontSize: 10)));
  }

  // Hourly labels
  Widget _hourLabel(int h) => Padding(padding: const EdgeInsets.only(top: 4), child: Text(h.toString().padLeft(2, '0')));
  Widget _bucketLabel(int i) {
    const ranges = ['0-3', '4-7', '8-11', '12-15', '16-19', '20-23'];
    final txt = (i >= 0 && i < ranges.length) ? ranges[i] : '';
    return Padding(padding: const EdgeInsets.only(top: 4), child: Text(txt, style: const TextStyle(fontSize: 10)));
  }

  // Line: workout minutes (hourly)
  Widget _buildWorkoutLineChartHourly(List<double> hourlyMins) {
    final spots = List.generate(24, (i) => FlSpot(i.toDouble(), hourlyMins[i].toDouble()));
    return SizedBox(
      height: 260, // more space to avoid collisions
      child: LineChart(
        LineChartData(
          minY: 0,
          titlesData: FlTitlesData(
            bottomTitles: AxisTitles(
              sideTitles: SideTitles(
                showTitles: true, interval: 4, getTitlesWidget: (v, _) => _hourLabel(v.toInt()), reservedSize: 28),
            ),
  ...
  Widget _buildCaloriesBarChartHourlyBuckets(List<Map<String, double>> hourly) {
    ...
    for (var h = 0; h < 24; h++) {
      final b = h ~/ 4;
      buckets[b]['in'] = (buckets[b]['in'] ?? 0.0) + (hourly[h]['in'] ?? 0.0);
      buckets[b]['out'] = (buckets[b]['out'] ?? 0.0) + (hourly[h]['out'] ?? 0.0);
    }
    final groups = List.generate(6, (i) {
      final inCal = (buckets[i]['in'] ?? 0.0).toDouble();
      final outCal = (buckets[i]['out'] ?? 0.0).toDouble();
      return BarChartGroupData(
        x: i,
        barsSpace: 8,
        barRods: [
          BarChartRodData(toY: inCal, color: Colors.orange, width: 10, borderRadius: BorderRadius.circular(3)),
          BarChartRodData(toY: outCal, color: Colors.green, width: 10, borderRadius: BorderRadius.circular(3)),
        ],
      );
    });
        ...
        SizedBox(
          ...
          child: BarChart(
            BarChartData(
              ...
              titlesData: FlTitlesData(
                bottomTitles: AxisTitles(
                  sideTitles: SideTitles(showTitles: true, interval: 1, getTitlesWidget: (v, _) => _bucketLabel(v.toInt()), reservedSize: 32)),
                leftTitles: AxisTitles(sideTitles: SideTitles(showTitles: true, reservedSize: 48)),
                topTitles: const AxisTitles(sideTitles: SideTitles(showTitles: false)),
                rightTitles: const AxisTitles(sideTitles: SideTitles(showTitles: false)),
              ),
  ...
  Widget _heroHeader({required String dateStr, required List<_KpiData> kpis, required bool loading, required String badge}) {
    return Container(
      decoration: BoxDecoration(
        ...
        boxShadow: [BoxShadow(color: AppTheme.primaryColor.withOpacity(0.25), blurRadius: 16, offset: const Offset(0, 8))],
      ),
      padding: const EdgeInsets.fromLTRB(16, 16, 16, 12),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text('Your Progress', style: const TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.bold)),
          const SizedBox(height: 4),
          Row(
            children: [
              Text(dateStr, style: TextStyle(color: Colors.white.withOpacity(0.9))),
              const Spacer(),
              Container(
                padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 6),
                decoration: BoxDecoration(color: Colors.white.withOpacity(0.2), borderRadius: BorderRadius.circular(20)),
                child: Text(badge, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w600)),
              ),
            ],
          ),
          const SizedBox(height: 12),
          if (loading)
            const SizedBox(height: 52, child: Center(child: CircularProgressIndicator(color: Colors.white)))
          else
            Row(
              children: [
      ...
      child: InkWell(
        ...
        child: AnimatedContainer(
          ...
          decoration: BoxDecoration(
            ...
            borderRadius: BorderRadius.circular(10),
            border: Border.all(color: selected ? AppTheme.primaryColor : Colors.black12, width: 1),
          ),
          ...
          child: Text(
            ...
            style: TextStyle(
              ...
              color: selected ? AppTheme.primaryColor : Colors.black87, // readable text
            ),
          ),
        ),
      ),
  ...
  Widget _motivationBanner({required List<String> messages}) {
    return Container(
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(16),
        boxShadow: [BoxShadow(color: AppTheme.primaryColor.withOpacity(0.18), blurRadius: 16, offset: const Offset(0, 8))],
      ),
        ...
        children: [
          Row(
            children: const [
              ...
              SizedBox(width: 8),
              Text('Motivation', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w800)),
            ],
          ),
          const SizedBox(height: 8),
  ...
  List<String> _buildMotivationMessages(Map<String, double> summary, NutritionGoal? goal, List<int> weeklyMins) {
    final calories = summary['calories'] ?? 0;
    final protein = summary['protein'] ?? 0;
    final dailyCalTarget = goal?.dailyCalories ?? 0;
    final pct = dailyCalTarget > 0 ? ((calories / dailyCalTarget) * 100).clamp(0, 999).toStringAsFixed(0) : '—';
    final needProtein = goal != null && goal.dailyProtein > 0 && protein < goal.dailyProtein * 0.8;
    final sessions = weeklyMins.where((m) => m > 0).length;
    final minsTotal = weeklyMins.fold<int>(0, (a, b) => a + b);

    return [
      if (dailyCalTarget > 0) 'You are $pct% toward today’s calorie goal — nice pace!',
      if (minsTotal > 0) 'Great work: ${minsTotal} min this week • ${sessions} sessions logged.',
      if (needProtein) 'Protein is a bit low — add a lean protein to your next meal.',
      if (dailyCalTarget == 0 && calories > 0) 'Calories logged today: ${calories.toStringAsFixed(0)} kcal — keep it up!',
    ];
  }
}
...
class _KpiData {
  ...
  final Color color;
  final IconData icon;
  _KpiData({required this.label, required this.value, required this.color, required this.icon});
}
        ...
        children: [
          ...
          Column(
            ...
            children: [
              ...
              Text(data.value, style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w800, color: Colors.black)), // force dark text
            ],
          ),
        ],
      ),
